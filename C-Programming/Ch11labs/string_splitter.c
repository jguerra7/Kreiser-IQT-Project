...
/*	Name: William Kreiser
	Date: 25 July 18
	Project: String Splitter
*/
char * split_the_string(char * string_ptr, char delimiter)
{
	if (!string_ptr) // If string_ptr is NULL, return error.
	{
		return ERROR_NULL_POINTER;
	}
	else if (!delimiter) //If delimiter is NULL, return error.
	{
		return ERROR_NULL_DELIMITER;
	}
	else
	{
		char * returnVal_ptr = string_ptr; //sets the return pointer equal to the original string
		...
		while (*(string_ptr + i) != '\0')
		{
			if (*(string_ptr + i) == delimiter)
			{
				...
				{	
					//sets the return pointer to the memory address of the string plus i plus 1
					returnVal_ptr = &*(string_ptr + i + 1);  
					delimiter_count++;
					position = i;
				}
				else
				{
					return ERROR_ABUNDANT_DELIMITER; //If delimiter greater than one, return error.
				}
			}
			i++;
		}
